...
├───────────────────────────────────┬──────────────────────────────────┤
│ [00:00:01] 各位，我们今天主要过三件… │  模式：会议 ▾        已总结至 12:10 │
│ [00:00:06] 我先说复盘，三季度营收…   │  最近一段                        │
│ [00:00:14] ▌那下季度目标我建议定在…  │  明确了留存优先，新版本 11/10 灰度 │
│                                   │  会议总览 / 纪要要点 / 待办事项    │
├───────────────────────────────────┴──────────────────────────────────┤
...
- 识别语言由识别服务端决定（应用不发送 `language` 字段）；上一句文本可作为 `prompt` 传给服务，提升人名/术语一致性
- **不区分说话人**：会议里可能不止一个人，也可能是线下会议或本地播放的音频，
  「我 / 对方」这种二分本来就不成立 —— 标错了比不标更糟，所以干脆不标
- **可疑结果只标注、不丢弃**：模型在静音上的幻觉、退化重复循环会被标灰并排除在纪要之外，
  但**始终显示在转写里**。识别出问题时，你需要看到原始结果，而不是一片空白

**AI 纪要**
- 播放中每 N 秒（默认 20s）滚动更新一次，**上下文长度恒定**，两小时会议也不会越来越慢、越来越贵
- **纪要不是实时字幕**：转写负责实时出字，纪要负责回头看 —— 它总结的是**已经过去的那一段时间**，
  面板上会显示「已总结至 12:10」这样的进度
- **两种模式，两套提示词**：
  - **会议**：结论、已达成的决定、待办（含负责人与截止时间）
  - **讲座 / 课程**：知识点、概念定义、讲者强调的重点、课后要复习的点
- 支持随时「立即总结」，停止后可生成完整结构化会议纪要
- 请求失败会**指数退避重试**（1s → 2s → 4s，最多 4 次）；本地小模型被压住而超时时，这一段内容
  不会丢 —— 没成功的部分不会推进「已总结到哪」，下一轮会重新送进去
...
```
已有纪要 + 新增转写（时间区间 A→B）──▶ 一次 LLM 调用 ──▶ 更新后的完整纪要（JSON）
                     ▲                                        │
                     └────────────────────────────────────────┘
```

纪要不是实时字幕：转写负责实时出字，纪要负责回头看。每轮只送
「已有纪要 + 这段时间的新增片段 + 最近 90 秒原文」，因此：

- 上下文长度恒定，长会议的成本与延迟不随时间增长
- 面板上显示「已总结至 12:10」，随时知道纪要覆盖到哪一刻
- 模式决定提示词：会议抓决定与待办，讲座抓知识点与脉络（两套 system 提示词 + 两份字段说明）
...
声源线程一旦退出（设备被拔掉、被别的程序独占、PulseAudio 掉线…），后面录下去只会得到寂静，
而用户看到的却是一份「还在录、但没有内容」的会议 —— 这比直接报错糟糕得多。
所以采集中断会立刻收尾：把已经录到的部分存好，并在界面上说清楚原因。

（对比：识别服务或 AI 接口报错**不会**停止录音 —— 那些是网络/服务端问题，
重试就好，音频本身还在。见下一条。）

### 5. 失败必须重试，不能「跳过就当做过」

AI 接口失败时**不推进**「已总结到哪」这个游标，所以没成功的那段内容下一轮还会被重新送进去；
同时请求层做指数退避重试（1s → 2s → 4s，最多 4 次）。漏掉的纪要补不回来，
而多等几秒没人会注意到。
...
- **不做说话人分离**：多人会议里无法区分是谁在说（需要声纹聚类，属于另一个量级的功能）
